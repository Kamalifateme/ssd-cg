<ul class="nav"><?php
							...
							<li class="b-b b-light <?php if($t->id == $this->uri->segment(3)){ echo "bg-light dk"; } ?>">
								<a href="<?=base_url()?>tickets/view/<?=$t->id?>">
									...
									<small class="block small text-muted"><?=ucfirst($this -> applib->get_any_field('users',array('id'=>$t->reporter),'username'))?> | <?=strftime(config_item('date_format'), strtotime($t->created));?> </small>
								</a> </li>
								<?php } } ?>
							</ul>