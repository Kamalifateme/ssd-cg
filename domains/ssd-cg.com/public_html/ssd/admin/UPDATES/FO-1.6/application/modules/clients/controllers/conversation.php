...
class Conversation extends MX_Controller {
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			redirect($this->input->post('r_url'));
				...
				$data['admins'] = $this->msg_model->get_all_records($table = 'users',
							$array = array(
											'role_id'=> '1','activated' => 1),$join_table = '',$join_criteria = '','created');
				$data['users'] = $this->msg_model->group_messages_by_users($this->tank_auth->get_user_id());
				$this->template
				->set_layout('users')
	...
	function _message_notification($user_to,$message){
		$email_message = $this -> applib ->get_any_field('email_templates',array('email_group' => 'message_received'), 'template_body');
			...
			$params['subject'] = '[ '.$this->config->item('company_name').' ]'.' '.lang('new_message_subject');
