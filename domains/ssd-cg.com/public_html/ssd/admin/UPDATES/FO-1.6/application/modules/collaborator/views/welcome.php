...
<section class="scrollable padder">
	...
	<?php
	$dept = $this -> applib->get_any_field('account_details',array('user_id'=>$user_id),'department');
	?>
...
<div class="col-lg-4">
   ...
   <img src="<?=base_url()?>resource/avatar/<?=$this->user_profile->get_profile_details($user_id,'avatar')?>" class="img-circle">
        ...
        <?php
            $deptid = $this -> applib->get_any_field('account_details',array('user_id'=>$user_id),'department');
            $deptname = $this -> applib->get_any_field('departments',array('deptid'=>$deptid),'deptname');

            $project_timers = $this -> db -> where('user',$user_id)-> get('project_timer') -> result();
            $task_timers = $this -> db -> where('user',$user_id)-> get('tasks_timer') -> result();
            $project_hours[] = array();
            $task_hours[] = array();
