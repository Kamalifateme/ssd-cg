...
<?php
$bug_comments = $this -> db -> where(array('bug_id'=>$bug)) ->order_by('date_commented','desc') -> get('bug_comments') -> result();
                  if (!empty($bug_comments)) {
                  foreach ($bug_comments as $key => $c) {
  $avatar = $this -> applib->get_any_field('account_details',array('user_id'=>$c->comment_by),'avatar');
  $role_id = $this -> applib->get_any_field('users',array('id'=>$c->comment_by),'role_id');
  $user_role = $this->tank_auth->user_role($role_id);
  $username = $this -> applib->get_any_field('users',array('id'=>$c->comment_by),'username');
