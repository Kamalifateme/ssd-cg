...
class Installer extends MX_Controller {
    ...
    function _installation_exists(){
        $installer_file = './resource/tmp/installed.txt';
    ...
    function verify(){
        ...
        if(json_decode($env_data) == NULL){
            ...
            $this->session->set_userdata('envato_username', $p['buyer']);
            ...
            if(!$this->_initialize_db(config_item('version'))){
                 $this->session->set_flashdata('message','Database import failed. Check the resource/tmp/'.config_item('version').'.sql');
                    redirect('installer/?step=3');
                }
        }
    ...
    function complete(){
        ...
        if(file_exists('./resource/tmp/'.config_item('version').'.sql')){
            unlink('./resource/tmp/'.config_item('version').'.sql');
        }
        redirect('');
    }
    ...
    function _get_db($version = NULL){
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL,UPDATE_URL.'/files/'.$version.'.sql');

        $fp = fopen('./resource/tmp/'.$version.'.sql', 'w+');
    ...
    function _create_admin_account(){
        ...
        $envato_username = $this->session->userdata('envato_username');
        ...
        $codata = array('value' => $envato_username);
        $this->db->where('config_key','envato_username')->update(Applib::$config_table,$codata);
    ...
    function _initialize_db($version = NULL){
         ...
         $templine = '';
        // Read in entire file
            $lines = file('./resource/tmp/'.$version.'.sql');
