...
<div class="modal-dialog animated fadeInDownBig">
	<div class="modal-content modal-lg">
		...
		<button type="button" class="close" data-dismiss="modal">&times;</button> <h4 class="modal-title"><?=lang('update_city')?></h4>
		</div>
				<style>
.bootstrap-timepicker{z-index:1152 !important;}.datepicker{z-index:1151 !important;}
		...
		<?php
								if (!empty($task_city)) {
				foreach ($task_city as $key => $city) { ?>
					<?php
			 $attributes = array('class' => 'bs-example form-horizontal');
          echo form_open(base_url().'hospital/edit_city',$attributes); ?>
		<div class="modal-body">
			 <input type="hidden" name="id" value="<?=$city->id?>">
				...
				<div class="col-lg-12">
					<input type="text" class="form-control"  value="<?=$city->city?>"  name="city" required>
				</div>
				</div>
