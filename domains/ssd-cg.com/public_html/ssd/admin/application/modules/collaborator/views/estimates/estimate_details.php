...
<section id="content">
	<section class="hbox stretch">
		...
		<p class="h4"><?=lang('all_estimates')?></p>
			...
			?>
				...
				<?=$this->config->item('default_currency')?> <?=number_format($this->user_profile->estimate_payable($e->est_id),2,$this->config->item('decimal_separator'),$this->config->item('thousand_separator'))?>
				</div> <br>
    ...
    <?php } } ?> 
					...
					<tr>
						...
						<td><?=$this->config->item('default_currency_symbol')?> <?=number_format($item->total_cost,2,$this->config->item('decimal_separator'),$this->config->item('thousand_separator'))?></td>
					</tr>
					<?php } } ?>
					...
					<tr>
						<td colspan="3" class="text-right no-border"><strong><?=lang('tax')?> - <?=$est_tax?>%</strong></td>
