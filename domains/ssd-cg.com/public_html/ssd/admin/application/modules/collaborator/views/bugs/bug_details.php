			...
			<?php
				...
				<header class="header bg-white b-b clearfix">
					<div class="row m-t-sm">
						...
						<ul class="dropdown-menu">
						<li><a href="<?=base_url()?>collaborator/bugs/mark_status?b=<?=$bug->bug_id?>&s=unconfirmed&ref=<?=$bug->issue_ref?>"><?=lang('unconfirmed')?></a></li>
						<li><a href="<?=base_url()?>collaborator/bugs/mark_status?b=<?=$bug->bug_id?>&s=confirmed&ref=<?=$bug->issue_ref?>"><?=lang('confirmed')?></a></li>	
						<li><a href="<?=base_url()?>collaborator/bugs/mark_status?b=<?=$bug->bug_id?>&s=progress&ref=<?=$bug->issue_ref?>"><?=lang('in_progress')?></a></li>
						<li><a href="<?=base_url()?>collaborator/bugs/mark_status?b=<?=$bug->bug_id?>&s=resolved&ref=<?=$bug->issue_ref?>"><?=lang('resolved')?></a></li>
						<li class="divider"></li>
						<li><a href="<?=base_url()?>collaborator/bugs/mark_status?b=<?=$bug->bug_id?>&s=verified&ref=<?=$bug->issue_ref?>"><?=lang('verified')?></a></li>
						</ul>
						</div>
...
<!-- Start Comments -->
					...
					<!-- comment form -->
						...
						<article class="comment-item media"> <a class="pull-left thumb-sm avatar"><img src="<?=base_url()?>resource/avatar/<?=$this->user_profile->get_profile_details($this->tank_auth->get_user_id(),'avatar')?>" class="img-circle"></a> <section class="media-body">
						<?php   
			...
			echo form_open(base_url().'collaborator/bugs/comment?bug='.$bug->bug_id, $attributes); ?>
					...
					foreach ($bug_comments as $key => $comment) { ?>
													...
													<article id="comment-id-1" class="comment-item">
														...
														<img src="<?=base_url()?>resource/avatar/<?=$this->user_profile->get_profile_details($comment->comment_by,'avatar')?>" class="img-circle"> </a>
