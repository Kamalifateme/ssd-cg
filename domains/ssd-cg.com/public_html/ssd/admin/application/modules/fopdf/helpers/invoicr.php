...
class invoicr extends tFPDF_rotation 
{
	...
	*                                                                              *
	*******************************************************************************/
	function invoicr($size='A4',$currency='€',$language='en',$type='invoice')
	{
		$this->columns = 5;
		$this->items = array();
		$this->totals = array();
		...
		$v = explode(".", phpversion()); $v = $v[1];
		if ($v > 4) { $this->font = 'dejavusanscondensed'; }
                
		$this->setLanguage($language);
		$this->setDocumentSize($size);
		$this->setColor("#222222");
                
		$this->tFPDF('P','mm',array($this->document['w'],$this->document['h']));
		$this->AliasNbPages();
	...
	*******************************************************************************/
	function Header()
	{
		...
		if(config_item('show_invoice_tax') == 'FALSE' || config_item('show_estimate_tax') == 'FALSE'){
		...
		if(config_item('show_invoice_tax') == 'TRUE' || config_item('show_estimate_tax') == 'TRUE'){
