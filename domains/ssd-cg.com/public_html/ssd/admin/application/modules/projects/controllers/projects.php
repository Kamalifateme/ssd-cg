...
class Projects extends MX_Controller {
	...
	function delete()
	{
		...
		$project = $this->input->post('project_id');
			...
			$project_tasks = $this->db->where(array('project' => $project))->get(Applib::$tasks_table)->result(); 
			foreach ($project_tasks as $task) {
				$filenames = $this->db->where(array('task' => $task->t_id))->get(Applib::$task_files_table)->result(); foreach ($filenames as $file) {
					unlink('./resource/project-files/'.$file->file_name);
				}
				$this->db->delete(Applib::$task_files_table, array('task' => $task->t_id)); 
			}
