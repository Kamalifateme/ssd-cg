  ...
  <ul class="list-group no-radius m-b-none m-t-n-xxs list-group-lg no-border">
    ...
    <li class="list-group-item">
      <a class="thumb-sm pull-left m-r-sm">
        ...
        <?php
          $user_email = $this->user_profile->get_user_details($a->user,'email');
          $gravatar_url = $this -> applib -> get_gravatar($user_email);
           if(config_item('use_gravatar') == 'TRUE' AND $this -> applib -> get_any_field(Applib::$profile_table,array('user_id'=>$a->user),'use_gravatar') == 'Y'){ ?>
