...
<section class="vbox"> <section class="scrollable padder">
	<ul class="breadcrumb no-border no-radius b-b b-light pull-in ">
		...
		<li class="active">
		<a href="<?=base_url()?>edit_about">ویرایش کاتالوگ
		</a>
		</li>
	</ul>
	...
	<section class="panel">
			<?php
			if (!empty($task_catalog)) {
			foreach ($task_catalog as $key => $catalog) { ?>
			<?php
			 $attributes = array('class' => 'bs-example form-horizontal');
             echo form_open(base_url().'about/edit_catalog',$attributes); ?>
			...
			<header class="panel-heading"> <i class="fa fa-pencil"></i> ویرایش کاتالوگ </header>
                ...
                <input type="text" name="address" value="<?=$catalog->address?>"  id="imageu157" class="form-control" required/></a>
				</div>
				</div>
