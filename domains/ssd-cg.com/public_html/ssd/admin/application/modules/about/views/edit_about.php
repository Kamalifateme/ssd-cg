...
<section class="vbox"> <section class="scrollable padder">
	<ul class="breadcrumb no-border no-radius b-b b-light pull-in ">
		...
		<li class="active">
		<a href="<?=base_url()?>edit_about"><?=lang('update_about')?>
		</a>
		</li>
	</ul>
	...
	<section class="panel">
			<?php
			if (!empty($task_about)) {
			foreach ($task_about as $key => $about) { ?>
			<?php
			 $attributes = array('class' => 'bs-example form-horizontal');
             echo form_open(base_url().'about/edit_about',$attributes); ?>
			 
			 
			<header class="panel-heading"> <i class="fa fa-pencil"></i> <?=lang('update_about')?> </header>

				<input type="hidden" name="id" value="<?=$about->id?>">
				<div class="form-group" style="padding:15px;">
				<label class="col-lg-12 control-label"><?=lang('description')?> <span class="text-danger"></span></label>
				<div class="col-lg-12">
				<textarea name="description" class="form-control editor"><?=$about->description?></textarea>
				</div>
				</div>
