...
<section class="vbox"> <section class="scrollable padder">
	<ul class="breadcrumb no-border no-radius b-b b-light pull-in ">
		...
		<li class="active">
		<a href="<?=base_url()?>mosha/edit_fani">ویرایش تکنولوژی نوین اطلاعات
		</a>
		</li>
	</ul>
	...
	<section class="panel">
			<?php
			if (!empty($fani_details)) {
			foreach ($fani_details as $key => $fani) { ?>
			<?php
			 $attributes = array('class' => 'bs-example form-horizontal');
             echo form_open(base_url().'mosha/edit_fani',$attributes); ?>
			 
			 
			<header class="panel-heading"> <i class="fa fa-pencil"></i>ویرایش تکنولوژی نوین اطلاعات</header>
				...
				<div class="col-lg-12">
				<textarea name="description" class="form-control editor"><?=$fani->description?></textarea>
				</div>
				</div>
				...
				<div class="col-lg-12">
					<input type="text" class="form-control"  name="url"  value="<?=$fani->url?>"  required >
				</div>
				</div>
                ...
                <input type="text" name="image"  id="imageu157" class="form-control" value="<?=$fani->image?>" /></a>
				</div>
				</div>
                ...
                <input type="text" name="file"  id="imageu159" class="form-control" value="<?=$fani->file?>" /></a>
				</div>
				</div>		
