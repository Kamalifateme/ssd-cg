...
<div class="modal-dialog animated fadeInDownBig">
	<div class="modal-content modal-lg">
		...
		<button type="button" class="close" data-dismiss="modal">&times;</button> <h4 class="modal-title"><?=lang('update_alephpa')?></h4>
		</div>
				<style>
.bootstrap-timepicker{z-index:1152 !important;}.datepicker{z-index:1151 !important;}
				 ...
				 ?>
		<?php
								if (!empty($task_alephpa)) {
				foreach ($task_alephpa as $key => $alephpa) { ?>
					<?php
			 $attributes = array('class' => 'bs-example form-horizontal');
          echo form_open(base_url().'drug/edit_alephpa',$attributes); ?>
		<div class="modal-body">
			 <input type="hidden" name="id" value="<?=$alephpa->id?>">
          			

				<div class="form-group">
					<label class="col-lg-12 control-label"><?=lang('alephpa')?> <span class="text-danger">*</span></label>
				<div class="col-lg-12">
					<input type="text" class="form-control"  value="<?=$alephpa->type?>"  name="type" required>
				</div>
				</div>
				<div class="form-group">
					<label class="col-lg-12 control-label"><?=lang('url')?> <span class="text-danger">*</span></label>
				<div class="col-lg-12">
					<input type="text" class="form-control"  value="<?=$alephpa->url?>"  name="url" required>
				</div>
				</div>
