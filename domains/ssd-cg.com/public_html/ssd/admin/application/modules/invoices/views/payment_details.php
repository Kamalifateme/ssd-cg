                    ...
                    <div style="font-size:10pt;">
                      ...
                      <div style="text-align:center;color:white;float:right;background:#78ae54;width: 25%;
                        ...
                        <span> <?=lang('amount_received')?></span><br>
                                <?php
                                $inv_cur = $this -> applib -> get_any_field('invoices', array('inv_id' => $i->invoice),'currency');
                                $cur = $this->applib->currencies($inv_cur); ?>

                        <span style="font-size:16pt;"><?=$cur->symbol?> <?=number_format($i->amount,2,$this->config->item('decimal_separator'),$this->config->item('thousand_separator'))?></span>
                          ...
                          <table style="width:100%;margin-bottom:35px;table-layout:fixed;" cellpadding="0" cellspacing="0" border="0">
                            ...
                            <tbody>
                              <tr style="border-bottom:1px solid #ededed">
                                ...
                                <td style="padding: 10px 10px 5px 10px;text-align:right;word-wrap: break-word;" valign="top">
                                  <span><?=$cur->symbol?> <?=number_format($this->user_profile->invoice_payable($i->invoice),2,$this->config->item('decimal_separator'),$this->config->item('thousand_separator'))?> (- <?=lang('tax')?>) </span>
                                </td>
                                <td style="text-align:right;padding: 10px 10px 10px 5px;word-wrap: break-word;" valign="top">
                                  <span><?=$cur->symbol?> <?=number_format($i->amount,2,$this->config->item('decimal_separator'),$this->config->item('thousand_separator'))?></span>
                                </td>
                              </tr>
                            </tbody>
                          </table>
