...
class Auth extends MX_Controller
{
	...
	function register()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$this->form_validation->set_rules('password', 'Password', 'trim|required|xss_clean|min_length['.config_item('password_min_length').']|max_length['.config_item('password_max_length').']');
	...
	function _activate_email($email,$data){
			...
			$user_email =  str_replace("{EMAIL}",$data['email'],$username);
			$user_password =  str_replace("{PASSWORD}",$data['password'],$user_email);
			$user_fullname =  str_replace("{FULLNAME}",$data['fullname'],$user_email);
			...
			$message = str_replace("{SITE_NAME}",config_item('company_name'),$user_password);
					...
					$this->email->from('info@unique-web.ir', 'اطلاعات دارویی پادرا');
					$this->email->to($email); 
