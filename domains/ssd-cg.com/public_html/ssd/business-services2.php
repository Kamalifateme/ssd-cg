...
<head>
	...
	<meta name="description" content="استراتژی رشد ssd |ssd"/>
...
<br><br>
			<a itemscope itemtype="http://schema.org/url" itemprop="business-services" href="<?php echo $path; ?>business-services" style="font-size:13pt;display:block;border-radius:6px;border:2px #fff solid;width:330px;text-align:center;padding:5px;">بازگشت به صفحه استراتژی رشد</a><br>
				...
				<br>
                <?php if($image==""){}else { ?>
								<img itemscope itemtype="http://schema.org/photo" itemprop="servicePhoto" alt="دفتر مشاوره اطلاع رسانی و خدمات کارآفرینی" src="<?php echo $image; ?>" style="border-radius:7px;width:640px;height:320px;border:5px #fff solid;" />
				<?php } ?>
				<br>
				...
				$query = "SELECT rating_number, FORMAT((total_points / rating_number),1) as average_rating FROM fx_post_rating WHERE post_id = 7".$id." AND status = 1";
				$result = $db->query($query);
				$ratingRow = $result->fetch_assoc();
				?>
                <input name="rating" value="0" id="rating_star" type="hidden" postID="7<?php echo $id?>" />
