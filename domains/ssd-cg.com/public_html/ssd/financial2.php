...
<br><br>
									<a itemscope itemtype="http://schema.org/url" itemprop="financialStrategy" href="<?php echo $path; ?>financial" style="font-size:13pt;display:block;border-radius:6px;border:2px #fff solid;width:200px;text-align:center;padding:5px;">بازگشت به صفحه استراتژی و بازاریابی</a><br>
				...
				$url2=$a[1];
				$url3=$a[2];
				$sql = "UPDATE  fx_mali  SET viwe=viwe+1 where url='$url2'";
				mysql_query($sql) or die(mysql_error());
				mysql_query("SET CHARACTER SET utf8");   
				mysql_query("SET NAMES utf8_persian_ci");
				$ses_sql=mysql_query("select * from  fx_mali where url='$url2' order by id DESC ") or die(mysql_error()) ;
				...
				<br>
                <?php if($image==""){}else { ?>
								<img itemscope itemtype="http://schema.org/photo" itemprop="downloadFileImage" src="<?php echo $image; ?>" style="border-radius:7px;width:6400px;height:320px;border:5px #fff solid;" />
				<?php } ?>
				<br>
				...
				$query = "SELECT rating_number, FORMAT((total_points / rating_number),1) as average_rating FROM fx_post_rating WHERE post_id = 5".$id." AND status = 1";
				$result = $db->query($query);
				$ratingRow = $result->fetch_assoc();
				?>
                <input name="rating" value="0" id="rating_star" type="hidden" postID="5<?php echo $id?>" />
