...
class BtBtPortfolioDataSource extends BTSource {
	...
	public function getList() {
		...
		$query .= " WHERE a.published = 1" . " AND a.access IN(" . implode(',', $user->authorisedLevels()) . ")" . " AND c.published = 1" . " AND c.access IN(" . implode(',', $user->authorisedLevels()) . ") AND a.language in (" . $db->Quote(JFactory::getLanguage()->getTag()) . "," . $db->Quote('*') . ")";
		// User filter
		$userId = $user->get('id');
		...
		foreach ($data as $key => &$item) {
			...
			if ($limitDescriptionBy == 'word') {
				...
				$item->description = self::substrword($item->description, $maxDesciption, $stringtags);
